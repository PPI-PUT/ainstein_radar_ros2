...
{
AinsteinRadarDriverK77::AinsteinRadarDriverK77(
  bool sendRaw, bool sendTracked, bool antennaType,
  std::string frameId)
{
...
void AinsteinRadarDriverK77::startRadar(
  ...
  const rclcpp::Time & time, const rclcpp::Logger & logger)
{
  ...
  if (m_sendRaw && m_sendTracked) {
    RCLCPP_ERROR(
      logger, "DATA TYPE PARAMETERS ARE BOTH SET TO TRUE,"
      "THE K-77 RADAR CAN ONLY PROCESS ONE TYPE AT A TIME"); return;
  }
  if (m_sendRaw) {m_can_frame_msg.data[2] = RADAR_SEND_RAW;} else if (m_sendTracked) {
...
void AinsteinRadarDriverK77::stopRadar(
  const rclcpp::Publisher<can_msgs::msg::Frame>::SharedPtr & publisher,
...
void AinsteinRadarDriverK77::updateRadarFOV(
  const rclcpp::Publisher<can_msgs::msg::Frame>::SharedPtr & publisher,
  const rclcpp::Time & time)
{
  m_can_frame_msg.header.stamp = time;
  ...
  if (m_antenaType) {m_can_frame_msg.data[4] = RADAR_SELECT_ANTENNA_LONG;} else {
    m_can_frame_msg.data[4] = RADAR_SELECT_ANTENNA_SHORT;
  }
  m_can_frame_msg.data[5] = RESERVED;
  m_can_frame_msg.data[6] = RESERVED;
  m_can_frame_msg.data[7] = RESERVED;
...
void AinsteinRadarDriverK77::msgCallback(
...
void AinsteinRadarDriverK77::processRadarReturn(
  ...
  const rclcpp::Logger & logger) const
{
  switch (command) {
    ...
    case RADAR_SELECT_ANTENNA:
      RCLCPP_ERROR(
        logger, "Received radar update antenna type message from radar with CAN ID %d",
        m_CAN_ID);
      break;
    default:
...
void AinsteinRadarDriverK77::processRadarStart(
  const rclcpp::Time & time,
  const rclcpp::Logger & logger)
{
...
void AinsteinRadarDriverK77::processRadarStop(
  const rclcpp::Publisher<radar_msgs::msg::RadarScan>::SharedPtr & publisher_raw,
  const rclcpp::Publisher<radar_msgs::msg::RadarScan>::SharedPtr & publisher_tracked,
  const rclcpp::Logger & logger)
{
  RCLCPP_ERROR(logger, "Received start frame from radar with CAN ID %d", m_CAN_ID);
  if (m_sendRaw) {publisher_raw->publish(m_radar_raw_data);} else if (m_sendTracked) {
