...
#include <utility>
  ...
  {
      AinsteinRadarDriverT79::AinsteinRadarDriverT79(bool sendRaw, bool sendTracked, int canId,
    ...
    void AinsteinRadarDriverT79::startRadar(const rclcpp::Publisher<can_msgs::msg::Frame>::SharedPtr& publisher,
                                            const rclcpp::Time& time, const rclcpp::Logger& logger)
    {
      ...
      if (m_sendRaw && m_sendTracked){ m_can_frame_msg.data[2] = RADAR_SEND_TRACKED_AND_RAW;}
      else if (m_sendRaw){ m_can_frame_msg.data[2] = RADAR_SEND_RAW;}
    ...
    void AinsteinRadarDriverT79::stopRadar(const rclcpp::Publisher<can_msgs::msg::Frame>::SharedPtr& publisher,
    ...
    void AinsteinRadarDriverT79::updateRadarFOV(const rclcpp::Publisher<can_msgs::msg::Frame>::SharedPtr& publisher,
                                            const rclcpp::Time& time)
    {
      m_can_frame_msg.header.stamp = time;
      m_can_frame_msg.dlc = 8;
    ...
    void AinsteinRadarDriverT79::msgCallback(const can_msgs::msg::Frame::SharedPtr &msg,
    ...
    void AinsteinRadarDriverT79::processRadarReturn(const uint8_t& command, const rclcpp::Logger& logger) const
    {
      switch (command) {
        case RADAR_START:
    ...
    void AinsteinRadarDriverT79::processRadarStart(const rclcpp::Time& time, const rclcpp::Logger& logger)
    {
    ...
    void AinsteinRadarDriverT79::processRadarStop(const rclcpp::Publisher<radar_msgs::msg::RadarScan>::SharedPtr& publisher_raw,
                                                   const rclcpp::Publisher<radar_msgs::msg::RadarScan>::SharedPtr& publisher_tracked,
                                                   const rclcpp::Logger& logger)
    {
      RCLCPP_INFO(logger, "Received start frame from radar with CAN ID %d", m_canId);
      ...
      else if (m_sendRaw){ publisher_raw->publish(m_radar_raw_data);}
